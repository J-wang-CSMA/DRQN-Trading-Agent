    ...
    def train_on_batch(self, minibatch, discount_factor):
        ...
        with tf.GradientTape() as tape:
            for sequence_data in minibatch:
                ...
                print(q_values_sequence)
                # Replace NaN values in q_values_sequence with zeros
                q_values_sequence = tf.where(tf.math.is_nan(q_values_sequence), tf.zeros_like(q_values_sequence), q_values_sequence)

                print("Q-values sequence after NaN replacement (if any):")
                print(q_values_sequence)
                ...
                for t in range(len(states)):
                    ...
                    for a in range(self.action_size):
                        ...
                        print(target_q_values_sequence)
                # Replace NaN values in target_q_values_sequence with zeros
                target_q_values_sequence = tf.where(tf.math.is_nan(target_q_values_sequence), tf.zeros_like(target_q_values_sequence), target_q_values_sequence)
                print("Target Q-values sequence after NaN replacement (if any):")
                print(target_q_values_sequence)
...
class TradingEnvironment(gym.Env):
    def __init__(self, raw_data, data, dates, initial_cash=10000000, trade_size=100000, commission_rate=0.001, spread=0.001):
        ...
        self.observation_space = gym.spaces.Box(low=0, high=1, shape=(data.shape[1] + 3,), dtype=np.float32)
        ...
        self.spread = spread
        self.position = [0, 0, 0, 0, 1, 0, 0, 0, 0]
        self.trade_size_array = [-10, -5, -2.5, -1, 0, 1, 2.5, 5, 10]
    ...
    def step(self, action):
        ...
        transaction_cost = 0
        if action != 4:
            transaction_cost = self.trade_size * abs(self.trade_size_array[action]) * self.commission_rate * self.spread
    ...
    def calculate_reward(self, action, open_price, transaction_cost=0):
            ...
            # Sell
            if action < 4: #Sell actions
                if hypothetical_unrealized_pnl >= self.trade_size * abs(self.trade_size_array[action]):
                    hypothetical_unrealized_pnl = hypothetical_unrealized_pnl - self.trade_size * abs(self.trade_size_array[action])  # Reduce position size by trade size
                    hypothetical_cash_balance = hypothetical_cash_balance + self.trade_size * abs(self.trade_size_array[action]) - transaction_cost
                else:
            ...
            # Buy
                ...
                if hypothetical_cash_balance <= 0:
                    pass
                elif hypothetical_cash_balance < self.trade_size *  abs(self.trade_size_array[action]) + transaction_cost:
                    hypothetical_unrealized_pnl = hypothetical_cash_balance - transaction_cost
                    hypothetical_cash_balance = 0
                else:
                    hypothetical_unrealized_pnl = hypothetical_unrealized_pnl + self.trade_size *  abs(self.trade_size_array[action])
                    hypothetical_cash_balance = hypothetical_cash_balance - self.trade_size *  abs(self.trade_size_array[action]) - transaction_cost

            hypothetical_unrealized_pnl = hypothetical_unrealized_pnl * (close_price / open_price)
    ...
    def update_position(self, action, open_price, transaction_cost):
        close_price = self.raw_data[self.current_step][3]
        trade_value = self.trade_size * abs(self.trade_size_array[action])
        if action < 4:
            if self.unrealized_pnl >= trade_value:
                self.unrealized_pnl = self.unrealized_pnl - trade_value
                self.cash_balance = self.cash_balance + trade_value - transaction_cost
            else:
            ...
            if self.cash_balance <= 0:
                pass
            elif self.cash_balance < trade_value + transaction_cost:
                self.unrealized_pnl - self.cash_balance - transaction_cost
                self.cash_balance = 0
            else:
                self.unrealized_pnl = self.unrealized_pnl + trade_value
                self.cash_balance = self.cash_balance - trade_value - transaction_cost
...
def test_agent(env, drqn, episodes=10):
    ...
    for episode in range(episodes):
        ...
        plt.title(f'Cumulative Returns Over 2020 - Episode {episode + 1}')
        plt.legend()
        plt.show()
...
if __name__ == "__main__":
    ...
    stock_symbol = 'SPY'
    raw_data, data, dates = get_financial_data(stock_symbol, start_date='2015-01-01', end_date='2020-12-31')
    train_data = data[dates <= '2020-01-01']
    test_data = data[dates > '2020-01-01']
    raw_train_data = raw_data[dates <= '2020-01-01']
    raw_test_data = raw_data[dates > '2020-01-01']
    ...
    state_size = train_data.shape[1] + 9
        ...
        for episode in range(13):
            ...
            for step in range(sequence_length):
                ...
                hypothetical_rewards = [
                    ...
                    transaction_cost= 100000 * 0.001 * 0.001 * abs(train_env.trade_size_array[action])
                    ) for action in range(train_env.action_space.n)
                ]
